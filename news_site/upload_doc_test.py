...
CREDENTIALS_PATH = './news_site/credentials.json'

# The ID of your Google Docs document
DOCUMENT_ID = '1_9b8Vvas_qFplemGpApUl1DG1MV5dnCfdUOhuY0AVUM'
...
def authenticate():
    ...
    if os.path.exists('./news_site/token.json'):
        creds = Credentials.from_authorized_user_file('./news_site/token.json', SCOPES)
    ...
    if not creds or not creds.valid:
        ...
        with open('./news_site/token.json', 'w') as token:
    ...
    requests = [
        {
            'insertText': {
                'location': {
                    'index': end_index - 1,  # Insert at the beginning of the document
                },
                'text': news + "\n",
            }
        },
...
def add_heading_to_document(service, document_id, heading_text, heading_level=1):
    ...
    requests = [
        {
            'insertText': {
                'location': {
                    'index': end_index - 1  # Position where the heading text will be inserted
                },
                'text': heading_text + "\n"
            }
        },
        {
            'updateParagraphStyle': {
                'range': {
                    'startIndex': end_index,
                    'endIndex': end_index + len(heading_text) +1
                },
                'paragraphStyle': {
...
if __name__ == '__main__':
    ...
    add_heading_to_document(service, DOCUMENT_ID, 'Love u mate', heading_level=1)
